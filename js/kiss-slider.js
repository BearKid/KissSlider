 ...
 * Date:2014/04/16
 */
function KissSlider(mySliderId,prevBtnId,nextBtnId){
	var winJq = $(window);
	var _thisObj = this;

	this.prevBtnId = prevBtnId;//"prev-btn";
	this.nextBtnId = nextBtnId;//"next-btn";
	...
	$("#" + this.prevBtnId).click(function(){
		_thisObj.prev();
	});
	$("#" + this.nextBtnId).click(function(){
		_thisObj.next();
	});
	return this;
}
 ...
 * @public
 */
KissSlider.prototype.move = function (nextCurIndex){
	...
	if(this.direction == "x"){
		...
		this.sliderLine.animate({"margin-left": marginLeft},this.speed);
	} else{
		var marginTop = -(nextCurIndex * this.unactivatedSliderHeight);
		this.sliderLine.animate({"margin-top": marginTop},this.speed);
	}
	this.curIndex = nextCurIndex;
	return this;
}
//下一张 next page
KissSlider.prototype.prev = function (){
	var nextCurIndex = this.curIndex - 1;
	return this.move(nextCurIndex);
}
//上一张 previou page
KissSlider.prototype.next = function (){
	var nextCurIndex = this.curIndex + 1;
	return this.move(nextCurIndex);
}
